...
type SERVER_DATA struct {
	...
	MutX_SM                *sync.RWMutex
	MutX_SMState			*sync.RWMutex
...
func (thisServer *SERVER_DATA) isLeader()(bool){

	resp:=false

	thisServer.MutX_SM.RLock()
	if thisServer.state == LEADER {
		resp = true
	}else{
		resp = false
	}
	thisServer.MutX_SM.RUnlock()
	return resp
}
...
func (appres *APPEND_ENTRIES_RESPONSE) makeResp(thisServer *SERVER_DATA, incmgreq APPEND_ENTRIES_REQUEST, appendDec bool) {
	...
	appres.AppendedIndex = incmgreq.LogToAdd.Index
