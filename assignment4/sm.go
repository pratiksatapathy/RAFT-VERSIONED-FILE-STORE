...
//			for i:=0;i<len(incomingReq.MoreLog) ;i++  {
//
//				thisServer.addThisEntry(incomingReq.MoreLog[i])
//
//			}
...
func (thisServer *SERVER_DATA) leaderAppendEntriesResponse(incomingRes APPEND_ENTRIES_RESPONSE) (actions []interface{}) {
	...
	if (thisServer.term < incomingRes.ResponderTerm) {
		...
		if incomingRes.AppendSuccess == false {
			...
			nexttobeindex := incomingRes.appendedIndexFrom - 1
			...
			if true {
				...
				//fmt.Println("MIU:",incomingRes.appendedIndexTo)
				thisServer.leaderStateAttrData.matchIndex[incomingRes.From_CandidateId] =
					thisServer.getLogAtIndex(incomingRes.appendedIndexTo)
...
//				}
				...
				thisServer.leaderStateAttrData.nextIndex[incomingRes.From_CandidateId] = incomingRes.appendedIndexTo + 1
